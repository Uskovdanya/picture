...
const mask = selector => {
  ...
  function createMask(event) {
    let matrix = '+7 (___) ___ __ __',
        ...
        // статичное, работает на основе матрицы. Берем все не цифры и заменяем пустым местом
...
__webpack_require__.r(__webpack_exports__);
const modals = () => {
  let btnPressed = false; // можно убрать false тогда переменная будет undefined что тоже допускается
    ...
    trigger.forEach(item => {
      item.addEventListener('click', e => {
        ...
        windows.forEach(item => {
          item.style.display = 'none';
          item.classList.add('animated', 'fadeIn'); // добавляем классы библиотеки animate
        });
...
const showMoreStyles = (trigger, wrapper) => {
  const btn = document.querySelector(trigger); // способ с классами
  //cards.forEach(card => {
  ...
  //});
  // способ с подгрузкой данных с сервера
  ...
  btn.addEventListener('click', function () {
    (0,_services_requests__WEBPACK_IMPORTED_MODULE_0__.getResource)('assets/db.json').then(res => createCards(res.styles)).catch(error => errorMessage());
    this.remove();
  }); //у стрелочной функции в обработчиках не будет ссылаться на тот элемн на котором произошло событие. У него нет this/ чтобы использовать его нужно сделать обычную функцию 
